  ...
  >
    ...
    <order-section-container title="应收账单">
      <bill-table
        ...
        @delRowEv="
          delIndex => {
            if(arOrderPriceList[index].list.length == 1){
              arOrderPriceList.splice(index,1)
            }
            else {
...
import BillTable from "./BillTableNew";
import BillOperateRecordsDialog from "./BillOperateRecordsDialog";
import addCustomerDial from "./addCustomerDial.vue";
// 需要父子通信的数据
const OrderKeys = [
...
export default {
  ...
  computed: {
    ...
    computedApAirFreight() {
      return this.apOrderPriceList.find(item => item.expenseName === "空运费");
    },
    computedAgent() {
  ...
  methods: {
    ...mapMutations("orderModule", ["setOrderPayways"]),
     getSelectOpt(){
                this.$http.get(this.$service.listCompanyByCustomerResponsibleId+`?orderId=${this.order.id}`).then(res=>{
                if(res.code == 200) {
                    this.apOrderPays = res.data
                }
            })
        },
    init() {
    ...
    async getCustomer(val) {
      ...
      await this.$nextTick();
      this.$refs[
        `arPriceTable-${this.arOrderPriceList.length - 1}`
      ][0].addListItem(val);
    },
    confirmAddBill() {
  ...
  watch: {
    arOrderPriceList: {
      handler(nv, ov) {
        let resultOrderPayway = {}
        let resultData = nv.map(i => i.list[0])
        resultData.forEach(i => { 
          resultOrderPayway[i.expenseUnitId] = i.payWay
        })
        this.setOrderPayways(resultOrderPayway)
      },
      deep:true
    }
  }
