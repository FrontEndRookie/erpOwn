<template>
  ...
  <div class="bill-table">
    <div
      v-if="expenseType == 1"
      style="color:black;margin:20px 0 30px 0;font-size: 16px;"
    >
      <span style="margin-right: 5px;font-weight: bold;"
        >结算对象{{ index + 1 }}:</span
      >
      {{ computedData[0].expenseUnitName }}
    </div>
    <div class="table-title">
      ...
      >
        <span>结算方式:</span>
        <div
          v-auth-field:[payWayText]="['104000_FC']"
          style="margin-right: 20px;transform: translateY(-6px);"
        >
          <el-select
            v-model="payWay"
            :disabled="$utils.getOrderDisableds(this.order).orderDisabled"
            placeholder="请选择结算方式"
            ...
            <el-option
              ...
              :disabled="
                expenseType == 1 &&
                  !(
                    computedApOrderPays &&
                    computedApOrderPays.some(
                      i =>
                        i.name == item.label ||
                        (i.name == '月结' && item.label == '月结买单')
                    )
                  )
              "
            >
      ...
      <p>{{ computedTitle }}核销金额：{{ computedTotalWoCny }}</p>
      <p>
    ...
    <div style="margin-top: 20px">
      <el-button
        :disabled="!showAddExpenseBtn"
        size="mini"
        @click="addListItem(false)"
        type="primary"
        >添加费用
      </el-button>
      <el-button
        ...
        type="primary"
        v-auth="['152004_B']"
        :disabled="[3, 4].includes(reconciliationStatus)"
        >修改账单</el-button
      >
      <el-button
        v-else
        ...
        @click="confirmBill"
        type="primary"
        :disabled="!showReconciliationBtn"
        >确认账单</el-button
      >
      <el-button
        size="mini"
        type="primary"
        v-auth="['152010_B']"
        v-if="![3, 4].includes(reconciliationStatus) && expenseType == 1"
        :disabled="
          !showReconciliationBtn && !showArEditBillBtn && financeStatus != 1
        "
      ...
      <el-button
        type="primary"
        size="mini"
        v-if="[3, 4].includes(reconciliationStatus)"
      ...
      <el-dropdown
        ...
        v-if="!showTotalWoCny"
        @command="exportBillNote"
      >
        <el-button size="mini" type="primary">
          拉账单<i class="el-icon-arrow-down el-icon--right"></i>
        </el-button>
        <el-dropdown-menu slot="dropdown" type="primary">
          <el-dropdown-item>导出普通对账单</el-dropdown-item>
          <!-- <el-dropdown-item>导出debit note</el-dropdown-item> -->
        </el-dropdown-menu>
      </el-dropdown>
    </div>
...
export default {
  ...
  created() {
    ...
    this.payWay = this.userOrderPayways[this.computedData[0].expenseUnitId];
    console.log(
      this.userOrderPayways,
      this.computedData[0].expenseUnitId,
      "datas"
    );
    this.confirmBillId = this.computedBillId;
    this.targetStatus = this.reconciliationStatus;
  },
  data() {
  ...
  computed: {
    ...
    computedApOrderPays() {
      return (
        this.apOrderPays.filter(
          item => item.companyName == this.computedData[0].expenseUnitName
        )[0] &&
        this.apOrderPays.filter(
          item => item.companyName == this.computedData[0].expenseUnitName
        )[0].payWayEnumList
    ...
    computedCurrencyTypes() {
      return this.currencyTypes.map((item, idx) => {
        return {
          ...item,
          disabled:
            !this.arOrderPriceList.some(i =>
              i.list.some(s => s.currency == item.value)
            ) &&
    ...
    invoiceError() {
      return this.computedBillInvoiceApplyInfos.find(item => item.invoiceError);
    },
    computedBillInvoiceApplyInfos() {
      return this.bill.billInvoiceApplyInfos.map(item => {
        ...
        const invoiceError =
          applyAmountOrgn *
            (this.expenseType == 1
              ? this.arExchangeRates[applyCurrency]
              : this.apExchangeRates) <
          invoicedAmountCny;
        return {
          ...item,
          invoiceError
    ...
    targetExpenseUnitId() {
      return this.computedData.filter(item => item.expenseUnitId)[0]
        .expenseUnitId;
    }
  },
  watch: {
    payWay(nv, ov) {
      this.data.forEach(item => (item.payWay = nv));
    }
  },
  methods: {
    async exportBillNote() {
      const {
        order: { id: orderId },
      ...
      try {
        const {
          ...
          billId,
          orderId,
          settlementCompanyId: this.targetExpenseUnitId
        });
    ...
    async confirmBill() {
      ...
      const res = await this.$http.post(this.$service.confirmBill, {
        orderId,
        billId,
        settlementCompanyId: this.targetExpenseUnitId
      });
      if (res.code == 200) {
        this.confirmBillId = res.data;
        this.$message.success("确认账单成功");
        await this.saveOrder({ isRefresh: true, showSuccessMsg: false });
      }
    },
    payWayText() {
    ...
    dealExchangeRate(target) {
      ...
      this.expenseType == 1
        ? (this.arExchangeRates = res)
        : (this.apExchangeRates = res);
    },
    ...
    dataListFilter(val) {
      ...
      if (val) {
        this.expenseNameList = this.computedExpenseCodes.filter(
          (item, index) => {
            if (
              item.expenseName.indexOf(val) > -1 ||
              item.pinyin.indexOf(val.toLowerCase()) > -1 ||
              item.pinyinInitial.indexOf(val.toLowerCase()) > -1
            ) {
              return true;
            }
          }
    ...
    agentChange({ val, index }) {
      ...
      const { value: expenseUnitId, label: expenseUnitName, settlementWay } =
    ...
    addListItem(data) {
      ...
      if (data) {
        hasIdData = {
          expenseUnitId: data.split("-")[1],
          expenseUnitName: data.split("-")[0]
        };
      } else {
        hasIdData = this.computedData.filter(item => item.expenseUnitId)[0];
      }

      const {
        order: { id: orderId, orderNo, payWay, waybillNo },
        expenseType,
        computedBillId
      } = this;
      this.$emit("addRowEv", {
        ...
        expenseName: "",
        expenseType,
        expenseUnitId: expenseType === 1 ? hasIdData.expenseUnitId : "",
        expenseUnitName: expenseType === 1 ? hasIdData.expenseUnitName : "",
        orderId,
        orderNo,
        payWay,
        price: "",
    ...
    async jump2confirmBill(isSave = true) {
      try {
        if (isSave && !this.showArEditBillBtn && this.financeStatus != 1) {
          await this.confirmBill();
        }
        const {
          order: { id: orderId }
        } = this;
        this.$router.push({
          name: "ConfirmBill",
          query: {
            billId: this.confirmBillId,
            orderId,
            companyId: this.targetExpenseUnitId,
            invoStatus: this.$store.state.userOrder.arOrderPriceList[this.index]
              .status
          }
        });
      } catch (error) {}
